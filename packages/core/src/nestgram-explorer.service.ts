...
import { NestGramBaseDiscovery, NestGramContextType, NestGramParamsFactory } from './context';
 ...
 */
@Injectable()
export class ExplorerService<T extends NestGramBaseDiscovery> extends Reflector {
  private readonly NestGramParamsFactory = new NestGramParamsFactory();
  ...
  private createContextCallback(instance: object, prototype: unknown, methodName: string) {
    return this.externalContextCreator.create<Record<number, ParamMetadata>, NestGramContextType>(
      instance,
      prototype[methodName],
      methodName,
      ROUTE_ARGS_METADATA,
      this.NestGramParamsFactory,
      STATIC_CONTEXT,
      undefined,
